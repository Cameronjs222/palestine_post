...
class Official():
    ...
    def find_official_by_id(cls, id):
        ...
        official = Official(
    id=results[0]['id'],
    official_id=results[0]['official_id'],
    title=results[0]['title'],
    short_title=results[0]['short_title'],
    api_uri=results[0]['api_uri'],
    first_name=results[0]['first_name'],
    middle_name=results[0]['middle_name'],
    last_name=results[0]['last_name'],
    suffix=results[0]['suffix'],
    date_of_birth=results[0]['date_of_birth'],
    gender=results[0]['gender'],
    party=results[0]['party'],
    twitter_account=results[0]['twitter_handle'],
    facebook_account=results[0]['facebook_account'],
    youtube_account=results[0]['youtube_account'],
    govtrack_id=results[0]['govtrack_id'],
    cspan_id=results[0]['cspan_id'],
    votesmart_id=results[0]['votesmart_id'],
    icpsr_id=results[0]['icpsr_id'],
    crp_id=results[0]['crp_id'],
    google_entity_id=results[0]['google_entity_id'],
    fec_candidate_id=results[0]['fec_candidate_id'],
    url=results[0]['url'],
    rss_url=results[0]['rss_url'],
    contact_form=results[0]['contact_form'],
    in_office=results[0]['in_office'],
    cook_pvi=results[0]['cook_pvi'],
    dw_nominate=results[0]['dw_nominate'],
    ideal_point=results[0]['ideal_point'],
    seniority=results[0]['seniority'],
    next_election=results[0]['next_election'],
    total_votes=results[0]['total_votes'],
    missed_votes=results[0]['missed_votes'],
    total_present=results[0]['total_present'],
    last_updated=results[0]['last_updated'],
    ocd_id=results[0]['ocd_id'],
    office=results[0]['office'],
    phone=results[0]['phone'],
    fax=results[0]['fax'],
    state=results[0]['state'],
    district=results[0]['district'],
    missed_votes_pct=results[0]['missed_votes_pct'],
    votes_with_party_pct=results[0]['votes_with_party_pct'],
    votes_against_party_pct=results[0]['votes_against_party_pct'],
    at_large=results[0]['at_large'],
    ...
    @classmethod
    def find_officials_by_username(cls, username):
        query = """
        SELECT * FROM officials WHERE twitter_handle LIKE %(username)s
        """
